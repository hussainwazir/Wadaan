...
var KendoGridActive = function (_dataa) {
    ...
    var colModel = [
...

        {
            field: "", width: 170,
            title: "Action",
            template: "# if (IsUpdatedSupplier == 'true') { #  <a style='font-size:20px;cursor:pointer;' onClick= EditDetail(this) title='Edit Supplier' <span class='fa fa-edit'></span></a>   # }  if (IsDeleteSupplier == 'true'){# <a style='font-size:20px;cursor:pointer;' onClick= deleteRecordByID('#=supplierID#')  title='Delete Supplier'<span class='fa fa-trash'></span></a> #}   #",
...
$("#in-active-supplier").on('click', function () {
    ...
    KendoGlobalAjax({ commandName: 'Supplier_Select', values: { Status: status }, CallBack: loadinActiveSupplierRecord });
});
...
var KendoGridSupplierInActiveRecord = function (_data) {
    ...
    BindkendoGrid('in-active-supplier-grid', 50, colModel, _data);
};
...
function LoadRecordByID(e) {
    ...
    var dataItem = grid.dataItem(row);
    window.location.href = '/PROJECTMANAGERAREA/ProjectManager/MSupplierDetail?supplierID=' + dataItem.supplierID + '';
}
function EditDetail(e) {
    ...
    var grid = $("#" + $grid).data("kendoGrid");
    var dataItem = grid.dataItem(row);
    window.location.href = '/PROJECTMANAGERAREA/ProjectManager/MSupplierSave?supplierID=' + dataItem.supplierID + '';
}
