...
$(document).ready(function () {
    ...
    if (IsCreateMeeting == 'false') {
    
        $('#btn-add-meeting').hide();
    }
...
var KendoGrid = function (_data) {
    ...
    var colModel = [
        ...
        { field: "meetingDate", title: "Meeting Date", width: 190, filterable: true },
        ...
        { field: "employeeName", title: "Employee Name", width: 170, filterable: true },
        {
           ...
           //  template: "   <a style='font-size:20px;cursor:pointer;' onClick= EditDetail(this) title='Edit Meeting' ><span class='fa fa-edit'></span></a>  <a style='font-size:20px;cursor:pointer;' onClick= deleteRecordByID('#=meetingID#')  title='Delete Meeting'><span class='fa fa-trash'></span></a>"
            template: "# if (IsUpdatedMeeting == 'true') { #  <a style='font-size:20px;cursor:pointer;' onClick= EditDetail(this) title='Edit Meeting' <span class='fa fa-edit'></span></a>   # } if (IsDeleteMeeting == 'true'){# <a style='font-size:20px;cursor:pointer;' onClick= deleteRecordByID('#=meetingID#')  title='Delete Meeting'<span class='fa fa-trash'></span></a> #}   #",
...
function LoadRecordByID(e) {
    ...
    window.location.href = '/PROJECTMANAGERAREA/ProjectManager/MMeetingDetail?meetingID=' + dataItem.meetingID + '';
}
...
function EditDetail(e) {
    ...
    var grid = $("#" + $grid).data("kendoGrid");
    var dataItem = grid.dataItem(row);
    window.location.href = '/PROJECTMANAGERAREA/ProjectManager/MMeetingSave?meetingID=' + dataItem.meetingID + '';
}
