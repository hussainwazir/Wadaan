...
$(document).ready(function () {
    ...
    $('#btn-add-ddl-record').on('click', function (e) {
        ...
        if ($("#txt-add-record").val() == '') {
            $("#txt-add-record").css('border-color', '#e74c3c');
        } else {
            ...
            if ($('#DDLName').val() == 'Country') {
                ...
                KendoGlobalAjax({ commandName: 'addDistrictTitle', values: { Title: $('#DropDownTitle').val(), UserID: userId, ProvinceID: $('#ddlProvinceName :selected').val() }, CallBack: '' });

                setTimeout(function () { LoadDistrict($("#ddlProvinceName").val()); }, 100);
            } else if ($('#DDLName').val() == 'Tribe') {
                KendoGlobalAjax({ commandName: 'addTribeTitle', values: { Title: $('#DropDownTitle').val(), UserID: userId, DistrictID: $('#ddlDistrictName :selected').val() }, CallBack: '' });

                setTimeout(function () { LoadTribe($("#ddlDistrictName").val()); $("#DropDownTitle").val(''); }, 100);
            } else if ($('#DDLName').val() == 'SubTribe') {
                KendoGlobalAjax({ commandName: 'addSubTribeTitle', values: { Title: $('#DropDownTitle').val(), UserID: userId, TribeID: $('#ddlTribeName :selected').val() }, CallBack: '' });
...
function loadDDLS(countryID, provinceID, tribeID, subTribeID, districtID) {
    ...
    //setTimeout(function () {
    //    $('#ddlProvinceName').val(provinceID);
    // //   $('#ddlProvinceName').change()
    //    setTimeout(function () {
    //        //$('#ddlProvinceName').val(provinceID);
    //        $('#ddlDistrictName').val(districtID);

    //      // $('#ddlDistrictName').change()
