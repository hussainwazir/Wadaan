...
function LoadKendoDates() {
    ...
    $("#MeetingEndDate").kendoDateTimePicker({
        value: new Date(),
        format: "dd/MM/yyyy h:mm:ss tt",
        parseFormats: ["MMMM yyyy"]
    });
...
var loadMeetingData = function (d) {
    ...
    setTimeout(function () {
        ...
        $('#MeetingEndDate').val(JSON.parse(d.Value)[0].meetingEndDate);
...
function loadClientDDl() {
    ...
    KendoGlobalAjax({ commandName: 'Task_SelectDDL_ByProjectId', values: { ProjectID: selectedProject }, CallBack: task_SelectDDL_ByProjectIdCallBack });
...
var task_SelectDDL_ByProjectIdCallBack = function(responseJSON){
    BindComboForDefault(JSON.parse(responseJSON.Value), $("#TaskID"), "Select Task");
}
