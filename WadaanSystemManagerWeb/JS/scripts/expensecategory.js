$(function () {
    ...
    $('#btnSave').on('click', function (e) {
        // alert('hi');
        if (!validateForm("frmExpenseCategory")) {
            return false;
        }

        $("#frmExpenseCategory").ajaxForm();
        var options = {
        ...
        $("#frmExpenseCategory").ajaxSubmit(options);
        //clearform();
    });
...
function editDetailFormatter(cellvalue, options, rowObject) {
    return "<span onclick=\"GlobalAjax({commandName:'getexpensecategorybyid',values:{id:'" + cellvalue + "'},CallBack:EditDetail})\"><i class=\"fas fa-edit gridButton\"><i></span>";
}

function deleteFormatter(cellvalue, options, rowObject) {
    return "<span onclick=\"deleteExpenseCategory('" + cellvalue + "')\"><i class=\"fas fa-trash  gridButton\"><i></span>";
}
...
var deleteExpenseCategory = function (id) {
    swal.fire({
        title: 'Are you sure?',
        ...
        //alert(id);
        if (restult.value) {
            GlobalAjax({ commandName: 'deleteexpensecategoryById', values: "{id:'" + id + "'}", CallBack: '' });
