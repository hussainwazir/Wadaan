...
$(function () {
    ...
    DatePickerSetting($('#dpincomeDate'));
    ...
    GlobalAjax({ commandName: 'listincome', values: '{}', CallBack: loadIncome });

    $('#btnSave').on('click', function (e) {
        // alert('hi');
        if (!validateForm("frmIncome")) {
            return false;
        }
...
var Grid = function (_data) {
    ...
    var colModel = [
        ...
        {
            name: 'incomeDate', align: "center", editable: false, sortable: true, stype: 'text', width: 65,
            formatter: 'date',
            formatoptions: {
                newformat: 'd/m/Y',
            },
...
function editDetailFormatter(cellvalue, options, rowObject) {
    return "<span onclick=\"GlobalAjax({commandName:'getincomebyid',values:{id:'" + cellvalue + "'},CallBack:EditDetail})\"><i class=\"fas fa-edit gridButton\"><i></span>";
}

function deleteFormatter(cellvalue, options, rowObject) {
    return "<span onclick=\"deleteIncome('" + cellvalue + "')\"><i class=\"fas fa-trash  gridButton\"><i></span>";
}
...
function EditDetail(d) {
    ...
    $('#incomeDate').val(dateFormat(data.incomeDate));
    $('#IncomeModel').modal();
}
...
var deleteIncome = function (id) {
    swal.fire({
        ...
        //alert(id);
        if (restult.value) {
            GlobalAjax({ commandName: 'deleteincomeById', values: "{id:'" + id + "'}", CallBack: '' });
